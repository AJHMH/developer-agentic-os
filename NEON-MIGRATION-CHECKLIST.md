...
- [ ] Run `cd /home/runner/work/developer-agentic-os/developer-agentic-os && DATABASE_URL=\"$DATABASE_URL\" CLERK_ORG_ID=\"$CLERK_ORG_ID\" npx tsx scripts/migrate-to-neon.ts` to apply `001-init.sql`, `003-hosted-deployment-normalization.sql`, and `004-hosted-state-normalization.sql`, provision the tenant, and import local data if present.
