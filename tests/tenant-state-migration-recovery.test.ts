...
test("deployment migration resolves legacy tenants per tenant, records status, and keeps sources for recovery", async () => {
  ...
  assert.match(sql, /FOR legacy_tenant_id IN\s+SELECT DISTINCT tenant_id FROM developer_agentic_os_vercel_project_mapping/);
  assert.match(
    sql,
  ...
  assert.match(sql, /IF cardinality\(failed_tenants\) = 0 THEN\s+DROP TABLE IF EXISTS developer_agentic_os_vercel_project_mapping;/);
});
...
test("deployment migration adds retry-safe dedupe guards for history and audit imports", async () => {
  ...
  assert.match(sql, /FROM developer_agentic_os_vercel_project_history AS legacy[\s\S]*ON CONFLICT DO NOTHING;/);
  assert.match(
    sql,
  ...
  assert.match(sql, /INSERT INTO vercel_webhook_audit \(tenant_id, action, vercel_project_id, occurred_at\)[\s\S]*ON CONFLICT DO NOTHING;/);
});
...
test("hosted state migration preserves workspace identity membership and deterministic retries", async () => {
  ...
  assert.match(sql, /FOR legacy_tenant_id IN\s+SELECT DISTINCT tenant_id FROM developer_agentic_os_workspace_state/);
  assert.match(
    sql,
  ...
  assert.match(sql, /IF cardinality\(failed_tenants\) = 0 THEN\s+DROP TABLE IF EXISTS developer_agentic_os_workspace_state;/);
  assert.match(
    sql,
...
test("migration guide documents recovery via per-tenant status and retained legacy tables", async () => {
  ...
  assert.match(guide, /if any tenant fails, the migration keeps\s+the legacy source table in place instead of dropping it/i);
